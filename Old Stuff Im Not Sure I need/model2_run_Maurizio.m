...
stock_sim = zeros([1 T]) + decision_func_to_use(dec_stock,state_vars(1,:),ssvals,sigma_Z,sigma_P);
risky_r_sim = zeros([1 T]) + decision_func_to_use(dec_risky_r,state_vars(1,:),ssvals,sigma_Z,sigma_P);
...
stock_returns = stock_r(stock_sim(2:T),stock_sim(1:T-1),profits_sim(2:T));
...
fprintf('\n\n Returns from stock prices\n\n');
fprintf('median return %.10g\nmedian riskless rate %.10g\n',median(stock_returns(1:T-1)),median(sophisticated_riskless_r(2:T)))
fprintf('mean return %.10g\nmean riskless rate %.10g\n',mean(stock_returns(1:T-1)),mean(sophisticated_riskless_r(2:T)))
